...
Copyright 2016-2020 Bella Pizzeria
*/

var eptlVersion = '1.7';
var eptlUpdated = '12.17.20';
var eptlDebug = false;
...
var notUserAction = false;
var userView = 'addItems';
var timeCheck;
var eptlClock;
var clockTaps;
var clockTapTimer;
...
$('body').on('click', '#btnFinishedOpening', function() {
	...
	var fodo = new Date();
	appStats.finishedOpening = formatDateString(fodo);
...
$('body').on('click', '#eptlVersion', function() {
	if(userView !== 'addItems') {
		...
		if($('#eptl-leftCol-addItems').children('.eptl-rowExpanded').length) {
			notUserAction = true;
			$('#eptl-leftCol-addItems').children('.eptl-rowExpanded')[0].click();
		}
	}
...
$('body').on('click', function() {
	if(!notUserAction) {
		...
		var luaStat = formatDateString(lastUserAction);
...
function setupTimeTable() {
	...
	$('#eptl-col-4').css('backgroundColor', hourColor(currentHour)).children('.eptl-row').css('backgroundColor', hourColor(currentHour)).children('h2').html((displayHour===0 ? '12' : displayHour)+':00 - '+((displayHour===12) ? '1' : displayHour+1)+':00');
	$('#eptl-col-3').css('backgroundColor', hourColor(HourBlock3)).children('.eptl-row').css('backgroundColor', hourColor(HourBlock3)).children('h2').html(HourBlock3+':00 - '+(displayHour===0 ? '12' : displayHour)+':00');
	$('#eptl-col-2').css('backgroundColor', hourColor(HourBlock2)).children('.eptl-row').css('backgroundColor', hourColor(HourBlock2)).children('h2').html(HourBlock2+':00 - '+HourBlock3+':00');
	$('#eptl-col-1').css('backgroundColor', hourColor(HourBlock1)).children('.eptl-row').css('backgroundColor', hourColor(HourBlock1)).children('h2').html(HourBlock1+':00 - '+HourBlock2+':00');
...
function lsLoad() {
	...
	setTimeout(function() {
		if(appSettings.newItemsTop === 'true') {
			...
			var lastAddedCol = $('#eptl-colData-4').children('.eptl-list');
			if(lastAddedCol.length) {
				var lastAddedItem = $(lastAddedCol).last().children().last().children('a')[0];
				var ot = $(lastAddedItem).offset().top;
				console.log('ot = '+ot);
				$('html,body').scrollTop(ot);
			} else {
				console.log('no items to scroll to');
				$('html,body').scrollTop(0);
			}
		}
	}, 10);
...
function lsAdd(iCat, iName, iQty) {
	...
	lastItemAdded = lsad;
	appStats.lastItemAdded = formatDateString(lastItemAdded);
	localStorage.setItem('eptlStats', JSON.stringify(appStats));
	refreshStats();
	if(eptlSocket) {
		var sInfo = {
...
function lsCleanup() {
	var validEntries = ['eptlSettings', 'eptlStats', today.toDateString()];
...
function initStats() {
	...
	if(localStorage.getItem('eptlStats') === null) {
		...
		appStats.store = ss;
		appStats.finishedOpening = checkFinishedOpening();
		appStats.runningSince = rs;
		appStats.deviceState = 'Active';
...
/* Determine if finishedOpening should be reset */
function checkFinishedOpening() {
	var rStat = 'No';
	var currentFo = appStats.finishedOpening;
	if(currentFo !== 'No') {
		var td = new Date();
		var tdds = td.toDateString();
		var fods = currentFo.split(' @ ')[0];
		if(tdds === fods) {
			finishedOpening = true;
			rStat = currentFo;
		}
...
function refreshStats() {
	...
	$('#statsDeviceState').html(appStats.deviceState);
}
      ...
      crossDomain: true,
			...
			success: function(result) {
				if(result==='success') {
					...
					var lastSyncTime = formatDateString(timeNow);
...
function initSocket() {
	...
	if(channel) {
		...
		if(!eptlSocket) {
			...
			eptlSocket = io('https://server.thebellapizza.com:3785', {transports: ['websocket']});
			...
			eptlSocket.on('getLocalData', function() {
				...
				for(var i=0; i<lsKeys.length; i++) {
					var lsk = lsKeys[i];
					if(lsk !== 'eptlSettings' && lsk !== 'eptlStats') {
			...
			eptlSocket.on('getStats', function() {
				eptlSocket.emit('postStats', appStats);
			});
			...
			eptlSocket.on('remoteMuteVolume', function() {
				remoteMuteVolume();
			});
			...
			eptlSocket.on('forceRestart', function() {
				var urlToLoad = cordova.file.applicationDirectory+'www/index.html';
	      window.location.href = urlToLoad;
			});

			eptlSocket.on('forceSync', function() {
				syncData(true);
				eptlSocket.emit('cbForceSync');
			});
			...
			eptlSocket.on('modSetting', function(info) {
				var msSetting = info.setting;
				var msValue = info.value;
				appSettings[msSetting] = msValue;
				localStorage.setItem('eptlSettings', JSON.stringify(appSettings));
				loadSettings();
				var pushSettings = {
					store: channel,
	...
	$.ajax({
		...
		success: function(result) {
			if(result.success === true) {
				...
				var dlf = new Date();
				appStats.lastFold = formatDateString(dlf);
				localStorage.setItem('eptlStats', JSON.stringify(appStats));
				refreshStats();
				setTimeout(function() {
...
/* Remote Mute Volume */
function remoteMuteVolume() {
	window.muteVolume(function(result) {
		if(result !== 'volume_muted') {
			eptlSocket.emit('volumeMuted', result);
		} else {
			eptlSocket.emit('volumeMuted', true);
		}
	});
}
...
function eptlCheckForUpdates() {
	...
	var lcfuTime = formatDateString(lcfuDate);
...
function formatDateString(dateObj) {
  var ts = dateObj.toTimeString().substr(0,8).split(':');
  var ampm = ' AM';
  var h = parseInt(ts[0]);
  if(h >= 12) ampm = ' PM';
  if(h > 12) {
  ...
  var rStr = dateObj.toDateString()+' @ '+h+':'+ts[1]+':'+ts[2]+ampm;
  return rStr;
}
